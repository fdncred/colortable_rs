...
pub fn print_table() {
    ...
    let ansi_bg_sequences = vec!["m", "40m", "41m", "42m", "43m", "44m", "45m", "46m", "47m", ];
    ...
    for (fg, fg_color) in ansi_fg_sequences.iter().enumerate() {
        ...
        for (bg, bg_item) in ansi_bg_sequences.iter().enumerate() {
            if bg > 0 { print!(" "); }
            if bg == 0 {
                cur_bg = format!("{}{}", ANSI_PREFIX, "40m");
            } else {
                cur_bg = format!("{}{}", ANSI_PREFIX, bg_item);
            }
